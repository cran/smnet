...
get_adjacency<-function(ssn_directory, net = 1){
  binaryIDs<-get_binaryIDs(ssn_directory, net = net)
  bid<-binaryIDs[,2]
  rid<-binaryIDs[,1]
  nch<-nchar(bid)
  ...
  add.one<-min(rid) == 0
  spam(list(j = (xy[,1]+add.one), i = (xy[,2]+add.one), rep(1, nrow(xy))), 
       nrow = n.segments, ncol=n.segments)
}	
